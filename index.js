...
const DEFAULT_CONTENT = {
  ...
  finalTitle: "Bạn là cả năm thanh xuân của tớ 🌷 , Yen Trang.",
...
function init() {
  ...
  mountInteractions();
  spawnAmbientHearts();
  const hasShareData = new URLSearchParams(window.location.search).has(SHARE_QUERY_KEY);
  if (hasShareData || hasCustomContent(state.content)) {
    syncShareUrl();
  }
}
...
function disableDevTools() {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    // Chặn Ctrl+Shift+K (Debugger)
    if (e.ctrlKey && e.shiftKey && e.key === "K") {
      e.preventDefault();
      return false;
    }
    // Chặn Ctrl+Shift+M (Responsive Design Mode)
  ...
  // Chặn các phím tắt inspect element
...
function loadContent() {
  const sharedContent = loadContentFromUrl();
  if (sharedContent) {
...
function syncShareUrl() {
  try {
    ...
    url.searchParams.set(SHARE_QUERY_KEY, JSON.stringify(state.content));
    ...
    // Keep local save even if URL update fails.
  }
}
...
function mountEditor() {
  ...
  refs.editorToggle.addEventListener("click", () => {
    setEditorOpen(!refs.editorPanel.classList.contains("is-open"));
  });
...
function setEditorOpen(isOpen) {
  refs.editorPanel.classList.toggle("is-open", isOpen);
  refs.editorPanel.setAttribute("aria-hidden", String(!isOpen));
  refs.editorToggle.textContent = isOpen ? "Close editor" : "Edit text";
}

function showEditedContent() {
  if (state.hideCardTimerId) {
    clearTimeout(state.hideCardTimerId);
    state.hideCardTimerId = null;
  }
  refs.finalStage.hidden = true;
  refs.finalStage.classList.remove("is-visible");
  refs.cardStage.hidden = false;
  refs.cardStage.classList.remove("is-hiding");
}
